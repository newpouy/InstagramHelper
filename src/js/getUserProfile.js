		...
		$.ajax({
			...
			error: function (jqXHR) {
				...
				if (jqXHR.status === 0) {
					...
					console.log('HTTP404 error getting the user profile.', new Date()); //eslint-disable-line no-console
					if (userId) {
						//alert('HTTP404 error - ' + userId);
						//console.log(userId);
						//console.log("new username", userId, username);
