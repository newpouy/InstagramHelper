...
followUser.unFollow = function (settings) {
  ...
  function errorUnFollow(jqXHR, resolve, reject) {
    console.log(`Error making ajax request to unfollow ${username}, status - ${jqXHR.status}`); //eslint-disable-line no-console
    console.log(arguments); //eslint-disable-line no-console
    var message;
    if (jqXHR.status === 0) {
      console.log('Not connected.', new Date()); //eslint-disable-line no-console
  ...
  function unFollow(userId, csrfToken, resolve, reject) {
    ...
    $.ajax({
      ...
      method: 'POST',
      success: function (data) {
        successUnFollow(data, resolve);
      },
      error: function (jqXHR) {
        errorUnFollow(jqXHR, resolve, reject);
      },
      headers: {
        'X-CSRFToken': csrfToken,
        'x-instagram-ajax': 1,
